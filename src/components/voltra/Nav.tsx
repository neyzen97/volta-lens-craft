...
export function Nav() {
  return (
    <motion.nav
      ...
      style={{
        background: "linear-gradient(to bottom, rgba(248,246,241,0.95) 0%, rgba(248,246,241,0) 100%)",
    ...
    >
      ...
      <div className="hidden md:flex gap-12 text-[10px] font-mono uppercase tracking-[0.25em] text-foreground/45">
        <a href="/#methode" className="hover:text-accent transition-colors duration-500">La Méthode</a>
        <a href="/#voix" className="hover:text-accent transition-colors duration-500">Témoignages</a>
      </div>

      <div className="flex items-center gap-4">
        <Link to="/mon-espace" className="hidden md:block font-mono text-[10px] uppercase tracking-[0.2em] text-foreground/40 hover:text-accent transition-colors duration-500">
          Mon espace
        </Link>
        <Link
          to="/inquiry"
