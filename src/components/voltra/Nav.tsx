  ...
  return (
    <>
      <motion.nav
        ...
        style={{ background: "linear-gradient(to bottom, rgba(13,12,10,0.9) 0%, rgba(13,12,10,0) 100%)", backdropFilter: "blur(12px)" }}
      >
        <Link to="/" className="font-serif text-xl md:text-2xl tracking-tight text-white hover:text-accent transition-colors duration-500">
          VOLTRA
        </Link>

        <div className="hidden md:flex gap-10 text-[10px] font-mono uppercase tracking-[0.25em] text-white/40">
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/inquiry"
            className="hidden md:block px-5 py-2.5 border border-white/20 text-[10px] font-mono uppercase tracking-[0.2em] text-white hover:bg-white hover:text-[#0D0C0A] transition-all duration-500"
          >
      ...
      {menuOpen && (
        ...
        >
          ...
          <Link to="/inquiry" onClick={() => setMenuOpen(false)}
            className="mt-4 px-8 py-4 bg-accent text-[#0D0C0A] font-mono text-[10px] uppercase tracking-[0.25em] font-semibold">
            Trouver mon photographe
          </Link>
        </motion.div>
      )}
